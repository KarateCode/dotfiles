alias ss='cd ~/.ssh'
alias s='~/dotfiles/scripts/ssh_aws.bash'

alias ls='ls -G'
alias la='ls -la'
alias b='cd -'
alias e='cd ~/appropos/envoy-web'

alias nr='npm run'
alias nt='npm test'
alias nrt='npm run test-one'
alias nrd='npm run dev'
alias nrs='npm run test:server'
alias nru='npm run test:utilities'
alias nrc='npm run test:client'
alias nrtd='npm run test-database'
alias td='npm run test-database 2>/dev/null'

alias cod='git checkout develop'
alias pd='git pull origin develop'
alias scour='~/dotfiles/scripts/scour.bash'
alias ns='cd ~/appropos/envoy-web/server && AWS_PROFILE=appropos ./bin/run-dev-server'
alias o='~/dotfiles/scripts/next_rebase_file.bash'
alias fd='~/dotfiles/scripts/next_git_diff_file.bash'
alias fa='~/dotfiles/scripts/next_git_add_file.bash'
alias fr='~/dotfiles/scripts/next_git_checkout_file.bash'
alias fo='~/dotfiles/scripts/next_git_ours_file.bash'
alias cf='~/dotfiles/scripts/next_vs_code_file.bash'
alias omf='git ls-files --others --exclude-standard | xargs sub && git ls-files -m | xargs sub'
alias reseed='npm run db:seed -- --email michael.schneider@envoyplatform.com'
alias ptq='env APPROPOS_ENV=local AWS_PROFILE=appropos AWS_REGION=us-east-1 npm run task process-tasks-queue'
alias task='AWS_PROFILE=appropos npm run task'
alias tasks='npm run tasks'
alias job='cd ~/appropos/envoy-web/server; env AWS_PROFILE=appropos APPROPOS_ENV=local ./bin/run-node bin/task-runner.js background-job-runner --integrationARN="arn:aws:sns:us-east-1:506597054164:appropos-integration-local-error" --ignoreJobInterval=true'

alias ds='node /Users/michaelschneider/code/tools-and-infrastructure/webdev-tools/menu.js'
alias gl='bash ~/dotfiles/scripts/grep-git-log.bash'
alias ccont='git cherry-pick --continue'
alias rcont='git rebase --continue; git status'
alias cmf='git ls-files --others --exclude-standard | xargs code; git ls-files -m | xargs code'

alias cb='git symbolic-ref --short HEAD'
alias gits='git status'
alias co='git checkout'
alias cob='git checkout -b'
alias gitc='git commit -m'
alias gitb='git branch'
alias gitd='git diff'
alias gita='git add -A . && git status'
alias gitl='git log'
alias gpull='git pull origin `git symbolic-ref --short HEAD` --rebase'
alias gpush='git push origin `git symbolic-ref --short HEAD`'
alias cof='bash ~/dotfiles/scripts/checkout_feature.bash'
alias stash='git add -A .; git stash; git status'
alias pop='git stash pop; git reset HEAD .; git status'
alias rmautodump='rm auto_dump_*.tar.gz; rm -rf auto_dump_*'
alias fcode='fzf | cut -d ":" -f 1 | xargs code'

alias gt="~/dotfiles/scripts/colorize_go_tests.bash"
alias gtr="~/dotfiles/scripts/colorize_go_test_run.bash"
alias lastc='git log -p -1'

alias conint='ssh -i ~/.ssh/con-envoy-stag-eu-west-1.pem ec2-user@ec2-52-16-118-244.eu-west-1.compute.amazonaws.com'
alias envint='ssh -i ~/.ssh/app-envoy-prod-us-east-1.pem ec2-user@ec2-52-4-132-185.compute-1.amazonaws.com'
alias awtint='ssh -i ~/.ssh/app-envoy-prod-us-east-1.pem ubuntu@ec2-23-23-175-155.compute-1.amazonaws.com'
alias wwwint='ssh -i ~/.ssh/www-envoy-vpc.pem ec2-user@ec2-54-174-109-36.compute-1.amazonaws.com'
alias appdev='ssh -i ~/.ssh/app-envoy-stag-us-east-1.pem ec2-user@52.6.239.36'
alias wwwdev='ssh -i ~/.ssh/www-envoy-vpc-uat.pem ec2-user@52.22.40.7'
alias condev='ssh -i ~/.ssh/con-envoy-stag-eu-west-1.pem ec2-user@52.30.49.202'

alias emacs='emacs -nw'
alias fix_know_hosts="cd ~/.ssh; sed -i '' '/UserKnownHostsFile/d' config; cd -"
alias onering='/Users/michaelschneider/code/tools-and-infrastructure/scripts/developer/one-ring/onering.sh'
