...
** Dirvish
C-x f launch
C-0 w copy full path of file to killring
...
* Omarchy:
Super + c/v # Copy/paste everywhere!
Super + Control + v # search clipboard
Hold Super + mouse # drags windows around
Super + l # toggles windows between tiling and scrolling
Super + f # fullscreen
Super + Alt + s # app to scratchpad
Super + s # toggles in and out of scratchpad
Super + g # groups apps
Super + alt + g # ungroup
YouTube has a hotkey
Create Web Apps on the fly!
Create TUIs on the fly!
Super + Control + O # toggle menu
Super + Shift + Backspace # gets rid of gaps between windows
Super + Control + t # floating top
Super + t # toggle as floating tile
Can read QR codes without a phone!
Has a 'Share' airdrop replacement thingie!!!
omacut is video editing
can download videos from the internet
Aether is preinstalled and will create a whole theme based on a wallpaper you give it
Double-click header bar makes it transparent
** keyd:
located at /etc/keyd/default.conf
sudo keyd reload # to bounce it
sudo keyd -m # watch keypresses in real time
...
tmux display-message -p "#{window_layout}" # outputs the goo that goes in tmuxinator scripts
...
hotkey < # brings up little menu to rearrange tabs (windows)
Left: swap-window -t -1
Right: swap-window -t +1
* Workmux
workmux was suggested by AI and endorsed by devops toolbox
workmux list
wm dashboard
wm sidebar
* Yassnippets
...
# Go into AWS Secrets manager to see all possible values
...
** Run integration on dev server
where's the job runner? scripts/run-task-for-env.sh
where do I drop files? /client/thorogood
SUBPREFIX=thorogood-na LOCK_ALERT_TIME_MINUTES=60 LOCK_ID="Mike_run" ./run_one.sh ./docker-run-task.sh background-job-runner --integrationARN=$INTEGRATION_ARN
...
* process-task-queue ptq:
Sometimes it takes a few minutes for the job to show up in SQS
* RipGrep
...
msa "ShipMethod.find({})" | jq '.[] | {code: .code, desc: .languages[].description}'
...
* Jira CLI:
michael.schneider@appropos.com
Configuration generated: /Users/michaelschneider/.config/.jira/.config.yml
jira issue list
jira issue list -a$(jira me)
jira issue comment add EP-13067 # (will prompt to open in emacs)
jira issue view EP-13221 --comments 20
jira epic list
jira sprint list
v to view a ticket
jira issue list -s"Dev Review""
jira issue move
jira issue move EP-13221 "Ready For Testing"

* New Relic:
Netsuite integration dataposts to our endpoints are keyed by requestId and stored in s3
...
To fix ^^, comment out in ssh config: UserKnownHostsFile=/dev/null
 - [ ]  Still need to figure out the fix for mbongo not connecting
...
rm -f .stylelintcache .eslintcache
...
* Slack:
Cmd+o to attach a file (opens in Downloads folder by default)
Cmd+/ opens the keyboard shortcut cheetsheet
Cmd+shift+/ react to last line
cmd+shift+h to start/stop huddle

* Switching between v41 and develop
nvm use
npm run install-dev
manually delete docker containers
npm run nginx-local
make sure database (percona) container is running
start BE
rebuild FE
...
* gh dash:
# zsh:
gh dash -c <(echo "prSections: [{title: 'PR #123', filters: '123'}]")
# nushell:
gh dash -c (mktemp --suffix .yml | do { let f = $in; "prSections: [{title: 'PR #7741', filters: 'is:pr 7741'}]" | save -f $f; $f })
...
supports csv toml json sqlite
awesome-nu # a repo with goodies and plugins
| explore # brings up a TUI to navigate around the data
| str replace "hello" "hi"
| lines # takes old school output like 'git status' and makes it NuShell structured
open sample.csv | select username password
open data.csv | columns
ms "Product.find({})" | get skus | flatten | get attributes | each { get 1 }
group-by
It's SOOOO close to Ruby syntax! Like only '|' instead of '.'
$'s to reference variables. So that the parse instantly knows if this a variable vs a string literal arg
Can convert to CSV or JSON! # | to json # | to csv
http get
...
* TV television:
tv list-channels
# fzf can preview files, but this can preview ANYTHING you configure it to
# Guess I need ideas:
# fzf preview
# man pages
# git differ: files on left, diff on right
# For anything that gives a list; then I'd have to copy and paste one by one to view them

* XLS notes:
open vba-template.xlsm
_Module1.vba has the hide/unhide vba function
upc_page.ts; console.log the identifiers (first array element) to see if we can dynamicall get upc col
https://envoyplatform.atlassian.net/wiki/spaces/EP/pages/3822452740/Libxl+Tips
node_modules/libxl/specs is a good place to look up libXL args
alerts in VBA are messageBox
README has documentation
open server/lib/b2b/offline-multi-order/vba-template.xlsm
click on developer tab; click on visual basic
in left file pane, expand Modules folder; double click on Module
vba logs to /Users/michaelschneider/Library/Containers/com.microsoft.Excel/Data

* Troubleshooting
"Could not load credentials from any providers". Check your AWS_PROFILE for unset or typos
"Credential should be scoped to a valid region." Did your config-overrides get copied into shared?
* WireGuard:
** Raw Tunnel
[Interface]
PrivateKey = 4NJIoVo+pixyWkEW+3Zc2pHRrzkgIuNo+KEqN/j8JFA=
Address = 10.8.0.5/24, fdcc:ad94:bacf:61a4::cafe:5/112
DNS = 1.1.1.1, 8.8.8.8, 8.8.8.4, 1.0.0.1
MTU = 1420
[Peer]
PublicKey = QYLoXSMmU4aocgGueLef282JiDHhCnwiCTQ9ock9aVs=
PresharedKey = 9VJnTkPPXdOO8Rc8ay18cUzXc9Ms6FQMDdBO4fkaen4=
AllowedIPs = 0.0.0.0/0
PersistentKeepalive = 0
Endpoint = ec2-44-216-27-206.compute-1.amazonaws.com:51820
** With IPV6 dropped:
[Interface]
PrivateKey = 4NJIoVo+pixyWkEW+3Zc2pHRrzkgIuNo+KEqN/j8JFA=
Address = 10.8.0.5/24
...
Endpoint = ec2-44-216-27-206.compute-1.amazonaws.com:51820
* Claude:
C-o to enter transcription viewer: C-d, C-u for pageUp/pageDown. C-n, C-p move by line
C-g pops $EDITOR
claude --resume # shows list that you can select from
claude --continue # jumps you straight into most recent
claude && osascript -e 'display notification "Claude may need input" with title "Claude CLI"'
* Herdr
Is a tmux replacement, but also has a left pane to manage your agents
prefix+g shows all agents
prefix+N new workspace
prefix+W rename workspace
prefix+X close entire tab
* Brow6el is a browser that runs in a terminal
* Sesh:
tmuxinator replacement
(integrates with zoxide)
workplace and context awareness
Can be triggered from Raycast
* Opencode
opencode --continue
opencode session list
opencode --session <sessionId>
C-x e opens in your editor for multi-line editing
Big Pickle is an LLM offered for free by opencode
Is /sessions how you go back to an old thread?
You can make agents, MD files that help AI do a specific task
Skills can be loaded on demand (just another prompt injection to guide)
Can pop your Editor!
     | Action | Keybind |
     |--------|---------|
     | Page Up | PageUp |
