...
def deleteMany [] {
    ...
    let delete_eval = "const result = db.getCollection('" + $collection_name + "').deleteMany({ _id: { $in: [" + $ids_array + "] } }); print('\\n\\x1b[32m  Deleted ' + result.deletedCount + ' document(s)\\x1b[0m');"

    mongosh --quiet $"mongodb://localhost:27017/($db_name)" --eval $delete_eval
